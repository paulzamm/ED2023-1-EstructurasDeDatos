...
public class Busqueda {
	...
	public static <T extends Comparable<T>> int buscarBinariaR(T dato, T[] vector) {
		return buscarBinariaR(dato, vector, 0);
	}
	
	private static <T extends Comparable<T>> int buscarBinariaR(T dato, T[] vector, int inf) {
		int centro, sup = vector.length - 1;
		if(inf<=sup){
			centro = (inf + sup) / 2;
			if (vector[centro].compareTo(dato) == 0)
				return centro;
				...
				inf = centro + 1;
			buscarBinariaR(dato, vector, inf+1);
		}
		return -1;
	}
